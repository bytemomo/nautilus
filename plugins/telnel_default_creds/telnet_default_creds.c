...
EXPORT int ORCA_Run(const char *host, unsigned int port, unsigned int timeout_ms, const char *params_json, char **out_json, size_t *out_len) {
#ifdef _WIN32
    WSADATA wsa;
            ...
            for (size_t j = 0; j < sizeof(default_passwords) / sizeof(default_passwords[0]); j++) {
                if (try_telnet_login(host, (uint16_t)port, default_users[i], default_passwords[j])) {
                    snprintf(report, sizeof(report),
                             ...
                             "%s/%s\"}] }",
                             default_users[i], default_passwords[j], (long long)now, (long long)now, default_users[i], default_passwords[j]);
                    goto done;
                }
            }
        }
