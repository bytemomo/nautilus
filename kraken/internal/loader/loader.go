...
func decodeTarget(cTarget *C.KrakenTarget) domain.Target {
	...
	case C.KRAKEN_TARGET_KIND_ETHERCAT:
		...
		return domain.EtherCATSlave{
			Interface:   C.GoString(ethercat.iface),
			Position:    uint16(ethercat.position),
			StationAddr: uint16(ethercat.station_addr),
			AliasAddr:   uint16(ethercat.alias_addr),
			VendorID:    uint32(ethercat.vendor_id),
			ProductCode: uint32(ethercat.product_code),
			RevisionNo:  uint32(ethercat.revision_no),
			SerialNo:    uint32(ethercat.serial_no),
			PortStatus:  uint16(ethercat.port_status),
		}
	default:
		return domain.HostPort{}
	}
