...
func runConformanceTests(ctx context.Context, mod *domain.Module, target domain.HostPort, res native.Resources, _ map[string]any, timeout time.Duration) (domain.RunResult, error) {
...
func buildFinding(moduleID string, target domain.HostPort, code, name string, err error) domain.Finding {
	success := err == nil
	severity := "info"
